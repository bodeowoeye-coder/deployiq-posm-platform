...
function formatStage(stage: string | null) {
  ...
  if (stage === "approved") return "Installed";
  return stage
    .split("_")
...
function installerStatusLabel(item: Submission) {
...
export default async function InstallerHistoryPage() {
  const context = await requireRole(["installer"], "/installer/history");
            ...
            <article key={item.id} className="grid min-w-0 gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-[88px_minmax(0,1fr)]">
              ...
              <div className="min-w-0">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-semibold">{installerStatusLabel(item)}</span>
                </div>
