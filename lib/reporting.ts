...
export function getDailyCounts(submissions: Submission[]) {
  ...
  submissions.forEach((item) => {
    const day = safeDateKey(item.submitted_at, "Unknown");
    ...
    reviewed.length === 0
      ...
      : Math.round(
          reviewed.reduce((sum, item) => {
            const reviewedAt = safeDate(item.reviewed_at);
            const submittedAt = safeDate(item.submitted_at);
            if (!reviewedAt || !submittedAt) return sum;
...
export function getInstallerAccuracyRanking(submissions: Submission[]) {
  ...
  return Array.from(buckets.values())
    .map((value) => ({ installer: value.installer, score: percentage(value.matched, value.total), total: value.total }))
...
export function getTrendSeries(submissions: Submission[]) {
  ...
  submissions.forEach((item) => {
    const date = safeDateKey(item.submitted_at, "Unknown");
