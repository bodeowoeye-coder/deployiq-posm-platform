...
export type ProvisioningPlan = {
  ...
  decisions: PlanDecision[]; warnings: string[]; approval: { required: boolean; reasons: string[] };
};
...
export type ShadowPlanningResult = { status: "completed" | "fallback" | "rejected" | "failed"; proposedPlan: ProvisioningPlan | null; baselinePlan: ProvisioningPlan; differences: PlanDifference[]; validation: PlanValidation; providerVersion: string; generatedAt: string; errorCode?: string };
